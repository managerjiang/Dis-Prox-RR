...
%% meth0_sw
% ==============需要读取.mat文件================
  load('data/G1_meth0_smote_sw_200.mat');
  load('data/X1_meth0_smote_sw_200.mat');
  load('data/G2_meth0_smote_sw_200.mat');%
  load('data/X2_meth0_smote_sw_200.mat');
  load('data/G3_meth0_smote_sw_200.mat');
  load('data/X3_meth0_smote_sw_200.mat');
  load('data/G4_meth0_smote_sw_200.mat');
  load('data/X4_meth0_smote_sw_200.mat');
...
%   obj_m0s(k)=fi/size(A,1)+lamuda1*norm(x_k_1,1)-fval;%+lamuda2*norm(x_k_1,2)^2; 
  ...
  obj_m0s(k)=(fv1+fv2+fv3+fv4)/4;
  zz_m0s(k)=(gv1+gv2+gv3+gv4)/4;
  %  XLX
  XLX=0;
  for i=1:agent_num
  ...
  % 梯度值
   %取所有智能体中第一个
%    g_k=gradient_sto{k}(:,1)+lamuda1*sign(x_k_1);
%    zz_m0s(k)=norm(g_k);
end
% 测试集
  load('w8a_test.mat');
...
%% meth1_sw
% ==============需要读取.mat文件================
  load('data/G_meth1_smote_sw_200.mat');%加载保存的迭代梯度信息x_k_store{}
  load('data/X_meth1_smote_sw_200.mat');%加载保存的迭代解信息gradient()
...
figure(1);
dxnum=60;
plot(1:5:dxnum,w_m0s(1:5:dxnum),'r-v','linewidth',1),hold on;
plot(1:5:dxnum,w_m1s(1:5:dxnum),'b-x','linewidth',1),hold on;
...
plot(1:10:Maxgen1s,zz_m1s(1:10:Maxgen1s),'b-x','linewidth',1),hold on;
...
plot(1:10:Maxgen1s,obj_m1s(1:10:Maxgen1s),'b-x','linewidth',1),hold on;
